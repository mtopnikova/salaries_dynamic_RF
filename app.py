...
my_df = df.loc[
    (df["Вид деятельности"] == areas[0])
    | (df["Вид деятельности"] == areas[1])
...
my_df["Годовой уровень инфляции"] = [
    0,
    18.58,
    15.06,
    11.99,
...
fig1.update_layout(
    title="Динамика номинальной среднемесячной ЗП работников в РФ за 2000-2023 гг",
    title_font_size=20,
    legend=dict(
        orientation="v", yanchor="top", y=0.95, xanchor="left", x=0.05, title_text=""
    ),
)
...
fig2.update_layout(
    title="Динамика реальной среднемесячной ЗП работников в РФ за 2000-2023 гг",
    title_font_size=20,
    legend=dict(
        orientation="v", yanchor="top", y=0.98, xanchor="left", x=0.01, title_text=""
    ),
)
...
fig3.update_layout(
    title="Динамика среднемесячной ЗП работников в сфере образования в РФ за 2000-2023 гг",
    title_font_size=14,
    legend=dict(
        orientation="v", yanchor="top", y=0.95, xanchor="left", x=0.05, title_text=""
    ),
...
newnames = {
    "Образование": "Номинальная ЗП",
    "Образование (с учетом инфляции)": "Реальная ЗП (с учетом инфляции)",
}
fig3.for_each_trace(
    lambda t: t.update(
...
fig4.update_layout(
    title="Динамика среднемесячной ЗП работников в сфере добычи полезных ископаемых в РФ за 2000-2023 гг",
    title_font_size=14,
    legend=dict(
        orientation="v", yanchor="top", y=0.95, xanchor="left", x=0.05, title_text=""
    ),
...
newnames = {
    "Добыча полезных ископаемых": "Номинальная ЗП",
...
newnames = {
    "Здравоохранение и предоставление социальных услуг": "Номинальная ЗП",
